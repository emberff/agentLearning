...
def create_msg(name: str, text: str, role: str = "user") -> Msg:
    """
    创建 AgentScope 2.x 兼容的消息
    
    Args:
        name: 发送者名称
        text: 消息内容文本
        role: 角色 (user, assistant 等，不建议用 system)
    
    Returns:
        Msg 对象
    """
    text_block = TextBlock(text=text)
    # AgentScope 2.x 不支持 system 角色，使用 user 替代
    if role == "system":
        role = "user"
    return Msg(name=name, content=[text_block], role=role)
...
def format_player_list(players: List[Agent], show_roles: bool = False) -> str:
    ...
    if show_roles:
        return "、".join([f"{p.name}({getattr(p, 'roles', '未知')})" for p in players])
    else:
